...
class SpotifyClient:
    ...
    def get_auth_token(self):
        ...
        response = requests.post(self.CONST_AUTH_URL,
    ...
    def get_user_auth(self):
        ...
        auth_url = f"{self.CONST_USER_AUTH_URL_BASE}client_id={self.client_id}&response_type=code&scope=playlist-read-private playlist-read-collaborative&redirect_uri={self.redirect_uri}&show_dialog=true"
    ...
    def get_user_playlists(self):
        playlist_url = self.CONST_USERS_BASE_URL + self.current_user_id + '/playlists'
        all_playlists_json = requests.get(playlist_url,{'limit': 50},
                                      headers={
        ...
        for playlist in all_playlists['items']:
            if playlist['owner']['id'] != self.current_user_id:
                continue
            print(playlist['id'] + ' - ' + playlist['owner']['id'])
